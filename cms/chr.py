...
class HistogramAccumulator:
    ...
    def predict_intervals(self, alpha, epsilon=None):
        ...
        S_grid = S_grid.reshape((len(self.alpha_grid),2))
        j_star_idx = np.where(self.alpha_grid <= alpha)[0]
        if len(j_star_idx)==0:
            lower = 0
            upper = self.K-1
        else:
            j_star_idx = np.max(j_star_idx)
            lower, upper = S_grid[j_star_idx,:]
    ...
    def calibrate_intervals(self, Y, epsilon=None, verbose=True):
        S_grid = self.compute_interval_sequence(epsilon=epsilon) 
        S_grid = S_grid.reshape((len(self.alpha_grid),2))
        band_left = self.breaks[S_grid[:,0]]
        band_right = self.breaks[S_grid[:,-1]]
        idx_inside = np.where((Y>=band_left)*(Y<=band_right))[0]
        if len(idx_inside)>0:
            alpha_max = self.alpha_grid[np.max(idx_inside)]
        else:
            alpha_max = 0
        score = 1.0-alpha_max
        return score
