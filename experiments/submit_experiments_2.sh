...
if [[ $CONF == 1 ]]; then
  ...
  CONFIDENCE_LIST=(95)
  METHOD_LIST=("bootstrap" "conformal-constant" "conformal-bootstrap")
  W_LIST=1000
  TWO_SIDED_LIST=(0 1)
  ...
  N_LIST=(1000000)
  CONFIDENCE_LIST=(50 95)
  METHOD_LIST=("classical" "bayesian-dp-?" "bootstrap" "conformal-adaptive")
...
#ORDP="sbatch --partition=sesia, --mem="$MEMO" --nodes=1 --ntasks=1 --cpus-per-task=1 --time="$TIME
                    ...
                    for UNIQUE in "${UNIQUE_LIST[@]}"; do
                      for N_BINS in "${N_BINS_LIST[@]}"; do
                        for N_TRACK in "${N_TRACK_LIST[@]}"; do
                          ...
                          OUT_FILE=$OUT_DIR_DATA"/detailed/exp2_"$JOBN".txt"
                          COMPLETE=0
                          #ls $OUT_FILE
                          if [[ -f $OUT_FILE ]]; then
                            COMPLETE=1
                          ...
                          if [[ $COMPLETE -eq 0 ]]; then
                            # Script to be run
                            SCRIPT="experiment_2.sh $SKETCH $DATA $D $W $N $METHOD $UNIQUE $N_BINS $N_TRACK $SEED $POSTERIOR $CONFIDENCE $TWO_SIDED"
